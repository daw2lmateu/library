...
$sql = "SELECT * FROM _31_members
        ...
        $sql = "SELECT COUNT(id) as booksReserved FROM _31_reservationsEbook WHERE member_id = '$userId'";
        $result = mysqli_query($conn,$sql);
        $data = mysqli_fetch_assoc($result);
        mysqli_free_result($result);
        mysqli_close($conn);

        if($data['booksReserved'] >= 5){   
            $_SESSION['message'] = "No puedes reservar mas de 5 libros electronicos";
            header("location: index.php");
        }
?>